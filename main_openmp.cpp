...
// Hàm max pooling cho đa kênh với OpenMP
...
// Hàm ReLU cho đa kênh với OpenMP
void relu_multi(std::vector<std::vector<std::vector<float>>>& input) {
    #pragma omp parallel for schedule(static, 1)
    for (int c = 0; c < input.size(); ++c) {
        for (int h = 0; h < input[c].size(); ++h) {
            for (int w = 0; w < input[c][h].size(); ++w) {
                input[c][h][w] = std::max(0.0f, input[c][h][w]);
            }
        }
    }
}
...
// Hàm fully connected với OpenMP
