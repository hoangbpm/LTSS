...
// Mã nguồn kernel OpenCL cho các phép toán CNN
        ...
        for (int c = 0; c < input_channels; c++) {
            for (int fy = 0; fy < filter_size; fy++) {
                for (int fx = 0; fx < filter_size; fx++) {
                    int in_x = x * stride + fx - half_filter;
                    int in_y = y * stride + fy - half_filter;
                    if (in_x >= 0 && in_x < input_width && in_y >= 0 && in_y < input_height) {
                        float input_value = input[(c * input_height + in_y) * input_width + in_x];
                        float filter_value = filter[((z * input_channels + c) * filter_size + fy) * filter_size + fx];
                        sum += input_value * filter_value;
                    }
                }
            }
        }
        sum += filter[output_channels * input_channels * filter_size * filter_size + z]; // Bias
...
// Định nghĩa cấu trúc mô hình CNN
struct CNNModel {
    int input_width, input_height, input_channels, filter_size, conv1_filters, pool_size, fc_size, num_classes;
    std::vector<float> conv1_weights, fc_weights, out_weights;
    int conv1_width, conv1_height, pool1_width, pool1_height;

    CNNModel(int width, int height, int channels, int fsize, int conv_filters, int p_size, int fc, int classes) 
        : input_width(width), input_height(height), input_channels(channels), filter_size(fsize),
          conv1_filters(conv_filters), pool_size(p_size), fc_size(fc), num_classes(classes) {
        
        conv1_width = input_width;
        conv1_height = input_height;
        ...
        float conv_scale = sqrt(2.0f / (input_channels * filter_size * filter_size));
        std::normal_distribution<float> conv_dist(0.0f, conv_scale);
        ...
        for (int i = 0; i < conv1_weights_size - conv1_filters; i++) conv1_weights[i] = conv_dist(gen);
        for (int i = conv1_weights_size - conv1_filters; i < conv1_weights_size; i++) conv1_weights[i] = 0.0f;

        int fc_input_size = conv1_filters * pool1_width * pool1_height;
    ...
    void save(const std::string& filename) {
        ...
        file.close();
        std::cout << "Da luu model vao " << filename << std::endl;
    }

    static CNNModel load(const std::string& filename) {
        std::ifstream file(filename, std::ios::binary);
        if (!file.is_open()) throw std::runtime_error("Khong the mo file: " + filename);
        ...
        CNNModel model(input_width, input_height, input_channels, filter_size, conv1_filters, pool_size, fc_size, num_classes);

        int conv1_size, fc_size_bytes, out_size;
...
private:
        ...
        if (model.input_channels == 1) {
            ...
            for (int y = 0; y < model.input_height; y++)
                for (int x = 0; x < model.input_width; x++) {
                    ...
                    input_data[(0 * model.input_height + y) * model.input_width + x] = pixel[2] / 255.0f;
                    input_data[(1 * model.input_height + y) * model.input_width + x] = pixel[1] / 255.0f;
                    input_data[(2 * model.input_height + y) * model.input_width + x] = pixel[0] / 255.0f;
                }
        }
        return input_data;
    }
    ...
    std::vector<float> forward(const std::vector<float>& input_data,
        ...
        conv2d_kernel.setArg(3, model.input_width);
        conv2d_kernel.setArg(4, model.input_height);
        conv2d_kernel.setArg(5, model.input_channels);
        conv2d_kernel.setArg(6, model.filter_size);
        conv2d_kernel.setArg(7, model.conv1_filters);
        conv2d_kernel.setArg(8, 1);
        queue.enqueueNDRangeKernel(conv2d_kernel, cl::NullRange, cl::NDRange(model.conv1_width, model.conv1_height, model.conv1_filters), cl::NullRange);
        ...
        queue.enqueueNDRangeKernel(max_pooling_kernel, cl::NullRange, cl::NDRange(model.pool1_width, model.pool1_height, model.conv1_filters), cl::NullRange);

        int fc_input_size = pool1_output_size;
        fc_kernel.setArg(0, d_pool1_output);
        ...
        queue.enqueueNDRangeKernel(fc_kernel, cl::NullRange, cl::NDRange(fc_output_size), cl::NullRange);
        ...
        queue.enqueueNDRangeKernel(fc_kernel, cl::NullRange, cl::NDRange(output_size), cl::NullRange);

        softmax_kernel.setArg(0, d_output);
        softmax_kernel.setArg(1, d_output);
        softmax_kernel.setArg(2, output_size);
        queue.enqueueNDRangeKernel(softmax_kernel, cl::NullRange, cl::NDRange(1), cl::NullRange);
    ...
    void backward(const std::vector<float>& input_data,
        ...
        queue.enqueueNDRangeKernel(cross_entropy_gradient_kernel, cl::NullRange, cl::NDRange(output_size), cl::NullRange);
        ...
        queue.enqueueNDRangeKernel(fc_gradient_kernel, cl::NullRange, cl::NDRange(fc_output_size + model.out_weights.size()), cl::NullRange);

        fc_gradient_kernel.setArg(0, d_pool1_output);
        ...
        fc_gradient_kernel.setArg(5, pool1_output_size);
        fc_gradient_kernel.setArg(6, fc_output_size);
        queue.enqueueNDRangeKernel(fc_gradient_kernel, cl::NullRange, cl::NDRange(pool1_output_size + model.fc_weights.size()), cl::NullRange);
        ...
        queue.enqueueNDRangeKernel(sgd_update_kernel, cl::NullRange, cl::NDRange(model.out_weights.size()), cl::NullRange);
        ...
        queue.enqueueNDRangeKernel(sgd_update_kernel, cl::NullRange, cl::NDRange(model.fc_weights.size()), cl::NullRange);
...
public:
    CNNTrainer(CNNModel& m, float lr = 0.001, int batch = 32, int e = 10)
        ...
        if (platforms.empty()) throw std::runtime_error("Khong tim thay nen tang OpenCL nao");
        cl::Platform platform = platforms[0];
        std::cout << "Nen tang OpenCL: " << platform.getInfo<CL_PLATFORM_NAME>() << std::endl;

        std::vector<cl::Device> devices;
        platform.getDevices(CL_DEVICE_TYPE_GPU, &devices);
        if (devices.empty()) platform.getDevices(CL_DEVICE_TYPE_CPU, &devices);
        if (devices.empty()) throw std::runtime_error("Khong tim thay thiet bi OpenCL nao");
        device = devices[0];
        std::cout << "thiet bi: " << device.getInfo<CL_DEVICE_NAME>() << std::endl;
    ...
    void train(const std::string& dataset_path) {
        ...
        std::cout << "Đã tải " << dataset.size() << " mau du lieu" << std::endl;
        ...
        std::cout << "Training set: " << train_set.size() << " mau" << std::endl;
        std::cout << "Validation set: " << val_set.size() << " mau" << std::endl;
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            for (size_t i = 0; i < train_set.size(); i += batch_size) {
                ...
                for (size_t j = i; j < batch_end; j++) {
                    ...
                    if (image.empty()) {
                        std::cerr << "Khong the tai anh: " << train_set[j].first << std::endl;
                        continue;
                    }
                    ...
                    std::vector<float> conv1_output, pool1_output, fc_output;
                    std::vector<float> output = forward(input_data, conv1_output, pool1_output, fc_output, forward_time_ms);
                    ...
                    backward(input_data, conv1_output, pool1_output, fc_output, output, targets, backward_time_ms);
                    total_backward_time += backward_time_ms;

                    if ((j + 1) % 10 == 0 || j + 1 == train_set.size()) {
                        std::cout << "\rDa xu ly " << j + 1 << "/" << train_set.size()
    ...
    int predict(const std::string& image_path, std::vector<float>& confidence, double& forward_time_ms) {
        ...
        if (image.empty()) throw std::runtime_error("Khong the tai anh: " + image_path);
...
int main() {
    try {
        int input_width = 32, input_height = 32, input_channels = 3, filter_size = 3, conv1_filters = 16, pool_size = 2, fc_size = 64, num_classes = 4;
        CNNModel model(input_width, input_height, input_channels, filter_size, conv1_filters, pool_size, fc_size, num_classes);
        CNNTrainer trainer(model, 0.001, 32, 10);

        int choice;
        std::cout << "Lua chon che do:\n1. Train model moi\n2. Tai va dung model da train\nNhap lua chon (1 hoac 2): ";
        std::cin >> choice;

        if (choice == 1) {
            std::string dataset_path;
            std::cout << "Nhap duong dan den file CSV dataset: ";
            std::cin >> dataset_path;
            trainer.train(dataset_path);
            model.save("model_final.bin");
        } else if (choice == 2) {
            std::string model_path, image_path;
            std::cout << "Nhap đuong dan den file model (.bin): ";
            std::cin >> model_path;
            model = CNNModel::load(model_path);
            std::cout << "Nhap duong dan đen anh can du doan: ";
            std::cin >> image_path;
            ...
            std::cout << "Du doan: " << class_names[predicted_class] << std::endl;
            std::cout << "Thoi gian forward pass: " << forward_time_ms << " ms" << std::endl;
